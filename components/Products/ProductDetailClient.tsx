...
export default function ProductDetailClient({ id }: ProductDetailClientProps) {
  ...
  return (
    <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top,rgba(14,165,233,0.14),transparent_34%),linear-gradient(180deg,#f8fafc_0%,#ffffff_72%)]">
      <div className="absolute inset-x-0 top-0 h-72 bg-[linear-gradient(180deg,rgba(255,255,255,0),rgba(255,255,255,0.7))]" />
      ...
      <Container className="relative py-8 sm:py-10 lg:py-14">
        <div className="space-y-8">
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)] xl:items-start">
            <ProductDetailGallery images={product.url} productName={product.name} />

            <div className="grid gap-6">
              <ProductDetailSummary product={product} />
              <ProductDetailPurchaseCard product={product} />
            </div>
          </div>
